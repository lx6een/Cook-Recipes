  ...
  try {
    ...
    modalBody.innerHTML = `
      <div class="recipe-modal-detail">
        ...
        <div>
          <h3 style="margin-bottom: 12px;">Пищевая ценность</h3>
      ...
      <div class="modal-instruction-block">
...
function handleRecipeCardClick(event, recipeId) {
  const checkbox = event.currentTarget.querySelector('.recipe-check');
      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div style="display: flex; gap: 8px; margin-top: 12px;">
            <a href="/recipes/${r.id}" class="btn btn-primary btn-sm" style="flex: 1; text-align: center;">Открыть</a>
            <button class="btn btn-danger btn-sm" style="flex: 1;" onclick="deleteRecipe(${r.id})">Удалить</button>
          </div>
        </div>
      </div>`
        )
...
async function initMealPlansPage() {
  ...
  function renderPlanCard(plan) {
    ...
    const recipesHtml = plan.recipes
      .map((r) => `<span class="tag clickable-recipe" onclick="openRecipeModal(${r.recipe_id})">${r.recipe_name} (${r.calories} ккал)</span>`)
      .join(" ");
  ...
  async function loadRecipesForSelect() {
    try {
      ...
      recipeCheckboxes.innerHTML = `<div class="recipes-selection-grid">
        ${allRecipes
          .map(
            (r) => `
        <div class="recipe-selection-card" onclick="handleRecipeCardClick(event, ${r.id})">
          <input type="checkbox" class="recipe-check" value="${r.id}" data-calories="${r.calories_per_100g}" onclick="event.stopPropagation()">
          <img class="recipe-select-image" src="${imageUrl(r.image)}" alt="${r.name}">
          <div class="recipe-select-body">
            <div class="recipe-select-title">${r.name}</div>
            <div class="recipe-select-meta">${formatDate(r.created_at)}</div>
            <div class="recipe-select-calories">${r.calories_per_100g} ккал/100г</div>
          </div>
        </div>`
          )
          .join("")}
